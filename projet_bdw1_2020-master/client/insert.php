    ...
    if( $_POST ) 
    {
        ...
        if($isSuccess && $isUploadSuccess) 
        {
            ...
            $maxPhtId = $result['MAX(photoId)'] + 1;
                        ...
                        /*Recuperation du uId de l'utilisateur */
            $stmt=$db->prepare("SELECT uId FROM Utilisateurs  WHERE nomU = :nomU");
            $stmt->bindValue(':nomU', $_SESSION['username']);
            $stmt->execute();
            $userId=$stmt->fetch();
            ...
            if($i==0)//Cas ou la categorie n'existe pas dans la base de donnée
            {
                ...
                /*Insertion des donnees de la nouvelle photo  Par defaut les images ajoutées le client sont visible pour tout le monde jusqu'a ce qu'il choisisse de les cacher */
                ...
                $stmt->bindValue(':uId', $userId['uId']);
                $stmt->bindValue(':visible','Oui');
                $stmt->execute();
